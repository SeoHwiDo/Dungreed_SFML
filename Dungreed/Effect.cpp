...
bool Effect::activate(const EffectSpawnRequest &request) {
    ...
    if (const auto pivot = resourceManager.getAnimationFirstFramePivot(request.atlasKey, request.animationName)) {
    ...
    m_animator = Animator{};
    m_animator.addAnimation(request.animationName, AnimationClip(frames, request.frameDuration, request.isLoop));
...
bool Effect::consumeHit(EntityId targetId) {
    if (m_finished || !m_isAttackEffect || m_hitTargets.find(targetId) != m_hitTargets.end()) {
        return false;
    }
