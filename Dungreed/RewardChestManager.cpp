...
namespace {
constexpr std::array<const char *, 4> kFairyAtlasKeys{"FairyS", "FairyM", "FairyL", "FairyXL"};
constexpr std::array<const char *, 4> kFairyFrameNames{"FairyS", "FairyM", "FairyL", "FairyXL"};

std::size_t getFairyIndex(FairyRewardSize size) { return static_cast<std::size_t>(size); }
} // namespace

bool RewardChestManager::init() {
    auto &resourceManager = ResourceManager::getInstance();
    const auto createSprite = [&resourceManager](const char *atlasKey, const char *frameName, bool bottomCenterOrigin) -> std::optional<sf::Sprite> {
        ...
        sprite.setOrigin(bottomCenterOrigin ? sf::Vector2f{frame->size.x * 0.5f, static_cast<float>(frame->size.y)} : sf::Vector2f{frame->size.x * 0.5f, frame->size.y * 0.5f});
        return sprite;
    };
    ...
    for (std::size_t index = 0; index < m_fairySprites.size(); ++index) {
        const std::vector<sf::IntRect> *frames = resourceManager.getAnimationFrames("Monster", kFairyAtlasKeys[index]);
        ...
        fairy.sprite.setOrigin(resourceManager.getAnimationFirstFramePivot("Monster", kFairyAtlasKeys[index]).value_or(sf::Vector2f{frames->front().size.x * 0.5f, frames->front().size.y * 0.5f}));
        fairy.animator.addAnimation(kFairyFrameNames[index], AnimationClip(frames, 0.1f, true));
...
void RewardChestManager::update(float dt, Room &room, const TileMap &tileMap, Player &player, EffectManager &effectManager, ObjectPoolingManager &objectPool) {
    ...
    if (&room != m_room || (m_state == State::Hidden && isRewardEligible(room) && !room.isClearRewardCollected())) {
    ...
    if (m_state == State::Closed) {
        ...
        const bool overlapsTreasure = treasureBounds && player.getGlobalBounds().findIntersection(*treasureBounds);
        if (overlapsTreasure && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::F)) {
            ...
            if (m_openHoldTime >= kOpenHoldDuration) {
                ...
                m_fairySprites[m_selectedFairyIndex]->sprite.setPosition(m_fairyStartPosition);
            }
        } else {
            m_openHoldTime = 0.f;
        }
    ...
    m_rewardElapsed += dt;
    if (m_selectedFairyIndex < m_fairySprites.size() && m_fairySprites[m_selectedFairyIndex]) {
        ...
        if (!m_isFairyInteractable) {
            ...
            fairy.sprite.setPosition(m_fairyStartPosition + (m_fairyTargetPosition - m_fairyStartPosition) * progress);
            m_isFairyInteractable = m_fairyRiseElapsed >= kFairyRiseDuration;
        }
    }
    const auto rewardBounds = getRewardBounds();
    if (m_isFairyInteractable && rewardBounds && player.getGlobalBounds().findIntersection(*rewardBounds)) {
...
void RewardChestManager::render(sf::RenderWindow &window) const {
    ...
    if (m_state == State::Closed && m_closedTreasureSprite) {
        ...
        if (m_openHoldTime > 0.f) {
            ...
            sf::RectangleShape progress({treasureBounds.size.x * (m_openHoldTime / kOpenHoldDuration), 3.f});
    ...
    if (m_state == State::RewardVisible && m_openedTreasureSprite && m_fairySprites[m_selectedFairyIndex]) {
...
void RewardChestManager::activateRoom(Room &room, const TileMap &tileMap, EffectManager &effectManager, ObjectPoolingManager &objectPool) {
    m_room = &room;
    m_openHoldTime = 0.f;
    m_rewardElapsed = 0.f;
    m_fairyRiseElapsed = 0.f;
    ...
    if (!isRewardEligible(room) || room.isClearRewardCollected() || !m_closedTreasureSprite || !m_openedTreasureSprite) {
        return;
    }
    ...
    const auto spawnIt = std::min_element(candidates.begin(), candidates.end(), [centerX, groundY](const sf::Vector2f &left, const sf::Vector2f &right) {
        const float leftScore = std::abs(left.x - centerX) + std::abs(left.y - groundY) * 10.f;
        const float rightScore = std::abs(right.x - centerX) + std::abs(right.y - groundY) * 10.f;
    ...
    m_fairySprites[m_selectedFairyIndex]->sprite.setPosition(room.isClearRewardChestOpened() ? m_fairyTargetPosition : m_fairyStartPosition);
    ...
    m_state = room.isClearRewardChestOpened() ? State::RewardVisible : State::Closed;
}

bool RewardChestManager::isRewardEligible(const Room &room) const { return room.getInfo().isClear && room.isMonsterEncounterPrepared() && !room.getEncounterMonsters().empty() && room.getClearRewardConfig().enabled; }
