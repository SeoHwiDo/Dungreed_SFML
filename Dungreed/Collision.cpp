...
std::optional<sf::Vector2f> Collision::checkHit(const sf::FloatRect &attackBox) const {
    ...
    return sf::Vector2f{
        intersection->position.x + intersection->size.x / 2.f,
...
void Collision::resolveMapCollision(Actor &actor, const TileMap &map,
    ...
    for (const TileData &tile : map.getCollisionTiles()) {
        if (tile.type == TileType::None ||
            (ignoreOneWay && tile.type == TileType::OneWay)) {
            continue;
        }
        ...
        if (tile.type == TileType::OneWay) {
            ...
            const bool crossedPlatformTop =
                previousBottom <= tile.bounds.position.y + groundEpsilon &&
        ...
        if (overlap.size.x < overlap.size.y) {
            actor.move(bounds.position.x < tile.bounds.position.x
                    ? -overlap.size.x
                    : overlap.size.x,
                0.f);
            movement.velocity.x = 0.f;
        } else if (movement.velocity.y >= 0.f &&
            bounds.position.y < tile.bounds.position.y) {
            actor.move(0.f, -overlap.size.y);
            movement.velocity.y = 0.f;
            movement.isGrounded = true;
...
bool Collision::resolveProjectileMapCollision(Projectile &projectile,
    const TileMap &map) {
    ...
    const unsigned int samples = std::max(1u,
        static_cast<unsigned int>(std::ceil(distance /
            std::max(1.f, std::max(tileSize.x, tileSize.y)))));

    for (unsigned int index = 1; index <= samples; ++index) {
        const float progress = static_cast<float>(index) / samples;
        ...
        for (const TileData &tile : map.getCollisionTiles()) {
            ...
            if (projectile.getGlobalBoundsAt(position).findIntersection(tile.bounds)) {
                projectile.setPosition(start + delta *
                    (static_cast<float>(index - 1) / samples));
                return true;
            }
        }
    }
