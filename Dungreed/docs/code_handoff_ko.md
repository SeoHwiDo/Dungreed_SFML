...
일반 전투방은 `room_data.json`의 `monster_pool`을 사용한다. 바깥 배열의 순서가 페이즈 순서이며, 각 내부 배열에는 `{ "monsterId", "count" }` 항목으로 해당 페이즈의 몬스터 종류와 수량을 명시한다. 몬스터 종류·수량·페이즈 수는 무작위로 선택하지 않는다. 첫 페이즈는 방 입장 후 요청하고, 소환 하이라이트가 끝난 뒤 행동한다. 현재 페이즈의 모든 몬스터가 사망 모션을 마치고 풀로 반환되면 `phaseDelay` 이후 다음 페이즈를 생성한다. 마지막 페이즈가 끝나야 방을 클리어 처리한다.
