...
std::uint8_t getLuminance(sf::Color color) {
...
sf::Color colorizeResult(sf::Color color, ResultType resultType) {
    ...
    return {
        luminance,
        static_cast<std::uint8_t>(std::min(255, luminance * 85 / 100)),
        static_cast<std::uint8_t>(std::min(255, luminance * 28 / 100)),
        color.a
    };
}
}
...
bool DeathScene::init(const sf::Font& font) {
    ...
    m_helpText.emplace(font,
        toSfUtf8String(u8"Enter키를 누르면 마을로 돌아갑니다"), 20);
...
bool DeathScene::enter(ResultType resultType) {
    ...
    const char* frameName = resultType == ResultType::Failure
        ? "ResultFail.png" : "ResultSuccess.png";
    ...
    m_messageText->setString(toSfUtf8String(resultType == ResultType::Failure
        ? u8"사망하였습니다"
        : u8"보스를 처치했습니다"));
    m_active = true;
...
void DeathScene::captureCurrentScreen() {
    if (!m_active || !m_captureRequested ||
        !m_snapshotTexture.resize(m_window.getSize())) {
        return;
    }
    ...
    for (unsigned int y = 0; y < size.y; ++y) {
        for (unsigned int x = 0; x < size.x; ++x) {
            image.setPixel({ x, y },
                colorizeResult(image.getPixel({ x, y }), m_resultType));
        }
    }
...
void DeathScene::render() {
    if (!m_active || !m_snapshotSprite || !m_resultSprite ||
        !m_messageText || !m_helpText) {
        return;
    }
    ...
    dim.setFillColor(m_resultType == ResultType::Failure
        ? sf::Color(0, 0, 0, 85)
        : sf::Color(90, 55, 0, 55));
    m_window.draw(dim);
...
void DeathScene::placeTexts() {
    ...
    for (const auto& [text, y] : {
             std::pair{ &*m_messageText, 185.f },
             std::pair{ &*m_helpText, 235.f } }) {
