...
bool hasDoor(const DoorPositions &positions, DoorPosition position) { return positions[static_cast<std::size_t>(position)]; }
...
void applyDoorways(RoomLayout &layout, const DoorPositions &positions) {
    ...
    if (outlineWidth == 0 || topBottomPassageWidth == 0 || sidePassageHeight == 0 || width < 2 * outlineWidth + topBottomPassageWidth || height < 2 * outlineWidth + sidePassageHeight + 1) {
        return;
    }

    const auto setCell = [&](unsigned int x, unsigned int y, RoomCell cell) { layout.cells[x + y * width] = cell; };
    ...
    if (hasDoor(positions, DoorPosition::Up)) {
        ...
        for (unsigned int x = centerStart; x < centerStart + topBottomPassageWidth; ++x) {
        ...
        setCell(centerStart + topBottomPassageWidth - 1, topWallY + 1, RoomCell::BackDoorTopRight);
    ...
    if (hasDoor(positions, DoorPosition::Down)) {
        ...
        setCell(centerStart + topBottomPassageWidth - 1, groundY - 1, RoomCell::BackDoorBottomRight);
    }
    ...
    if (hasDoor(positions, DoorPosition::Left)) {
        ...
        setCell(outlineWidth + 1, sideDoorTop, RoomCell::BackDoorTopLeft);
...
RoomLayout makeStyledRoom(unsigned int width, unsigned int height, std::initializer_list<PlatformSpec> platforms) {
    RoomLayout layout;
    layout.width = width;
    ...
    for (const PlatformSpec &platform : platforms) {
        for (unsigned int offset = 0; offset < platform.length; ++offset) {
            ...
            if (x < width && platform.y < height && layout.cells[x + platform.y * width] == RoomCell::BackTile) {
...
Room::Room(RoomType type, DoorPositions doorPositions, std::vector<RoomMonsterSpawn> monsterSpawns) {
...
void Room::loadReference(RoomType type, DoorPositions doorPositions) { loadLayout(type, getReferenceLayout(type), doorPositions); }

void Room::loadLayout(RoomType type, RoomLayout layout, DoorPositions doorPositions, std::vector<DecorativeTileConfig> decorations, std::vector<BackgroundLayerConfig> backgroundLayers) {
    info.type = type;
    info.isClear = false;
    ...
    for (unsigned int y = 0; y < info.layout.height; ++y) {
        for (unsigned int x = 0; x < info.layout.width; ++x) {
            if (!info.playerSpawnCell && info.layout.cells[x + y * info.layout.width] == RoomCell::SpawnPoint) {
...
void Room::prepareMonsterEncounter(std::vector<RoomMonsterSpawn> monsters, int phaseCount) {
...
void Room::setClear(bool isClear) { info.isClear = isClear; }

void Room::setTraversalLocked(bool locked) { info.traversalLockOverride = locked; }

bool Room::isTraversalLocked() const { return info.traversalLockOverride || (info.isEncounterInitialized && !info.isClear); }

std::optional<DoorPosition> Room::getEnteredDoor(const sf::FloatRect &actorBounds, const sf::FloatRect &previousActorBounds, const TileMap &tileMap) const {
    ...
    if (tileSize.x <= 0.f || tileSize.y <= 0.f || outlineWidth == 0 || topBottomPassageWidth == 0 || sidePassageHeight == 0 || info.layout.width < 2 * outlineWidth + topBottomPassageWidth || info.layout.height < 2 * outlineWidth + sidePassageHeight + 1) {
        return std::nullopt;
    }

    const unsigned int centerStart = (info.layout.width - topBottomPassageWidth) / 2;
    ...
    const auto makeBounds = [&tileSize](unsigned int x, unsigned int y, unsigned int width, unsigned int height) { return sf::FloatRect({static_cast<float>(x) * tileSize.x, static_cast<float>(y) * tileSize.y}, {static_cast<float>(width) * tileSize.x, static_cast<float>(height) * tileSize.y}); };
    const std::array<std::pair<DoorPosition, sf::FloatRect>, 4> triggers{std::pair{DoorPosition::Up, makeBounds(centerStart, outlineWidth - 1, topBottomPassageWidth, 2)}, std::pair{DoorPosition::Down, makeBounds(centerStart, groundY, topBottomPassageWidth, 2)}, std::pair{DoorPosition::Left, makeBounds(0, sideDoorTop, outlineWidth + 1, sidePassageHeight)}, std::pair{DoorPosition::Right, makeBounds(rightWallX, sideDoorTop, info.layout.width - rightWallX, sidePassageHeight)}};

    const auto intersectsMovement = [&](const sf::FloatRect &triggerBounds) {
        if (actorBounds.findIntersection(triggerBounds) || previousActorBounds.findIntersection(triggerBounds)) {
            return true;
        }

        const sf::Vector2f start{previousActorBounds.position.x + previousActorBounds.size.x * 0.5f, previousActorBounds.position.y + previousActorBounds.size.y * 0.5f};
        const sf::Vector2f end{actorBounds.position.x + actorBounds.size.x * 0.5f, actorBounds.position.y + actorBounds.size.y * 0.5f};
        const float halfWidth = std::max(actorBounds.size.x, previousActorBounds.size.x) * 0.5f;
        const float halfHeight = std::max(actorBounds.size.y, previousActorBounds.size.y) * 0.5f;
        const sf::FloatRect expandedTrigger{{triggerBounds.position.x - halfWidth, triggerBounds.position.y - halfHeight}, {triggerBounds.size.x + halfWidth * 2.f, triggerBounds.size.y + halfHeight * 2.f}};
        const sf::Vector2f delta = end - start;
        float minimumTime = 0.f;
        float maximumTime = 1.f;

        const auto intersectsAxis = [&](float origin, float direction, float minimum, float maximum) {
        ...
        return intersectsAxis(start.x, delta.x, expandedTrigger.position.x, expandedTrigger.position.x + expandedTrigger.size.x) && intersectsAxis(start.y, delta.y, expandedTrigger.position.y, expandedTrigger.position.y + expandedTrigger.size.y);
    };

    for (const auto &[position, triggerBounds] : triggers) {
        const std::size_t index = static_cast<std::size_t>(position);
        if (index < info.doors.size() && info.doors[index].isOpen && info.doors[index].next && intersectsMovement(triggerBounds)) {
            return position;
        }
    }
    return std::nullopt;
...
void Room::setDoorNext(DoorPosition position, Room *nextRoom) {
    const std::size_t index = static_cast<std::size_t>(position);
    if (nextRoom != this && index < info.doors.size() && (!info.doors[index].next || info.doors[index].next == nextRoom)) {
...
bool Room::buildTileMap(TileMap &tileMap, const std::string &tileAtlasKey, const RoomTileSet &tileSet) const {
    ...
    const auto getCell = [&](int x, int y) {
        if (x < 0 || y < 0 || x >= static_cast<int>(info.layout.width) || y >= static_cast<int>(info.layout.height)) {
            return RoomCell::Empty;
        }
        return info.layout.cells[static_cast<unsigned int>(x) + static_cast<unsigned int>(y) * info.layout.width];
    };
    // 백타일은 시각 전용이므로 충돌 없이 배경 버텍스 배열에 넣습니다.
    const auto makeBackTile = [](const std::string &frameName) { return TileConfig{frameName, TileType::None, true}; };
    // 벽과 맞닿는 방향 및 문 입구의 모서리를 기준으로 적합한 그림자 백타일 프레임을 선택합니다.
    const auto getBackFrame = [&](unsigned int x, unsigned int y, RoomCell cell) -> const std::string & {
        const unsigned int outlineWidth = info.layout.outlineWidth;
        const unsigned int topBottomPassageWidth = info.layout.topBottomPassageWidth;
        const unsigned int sidePassageHeight = info.layout.sidePassageHeight;
        const unsigned int topWallY = outlineWidth;
        ...
        if (cell == RoomCell::Door) {
            const auto isWall = [](RoomCell neighbor) { return neighbor == RoomCell::Ceiling || neighbor == RoomCell::Ground || neighbor == RoomCell::LeftWall || neighbor == RoomCell::RightWall || neighbor == RoomCell::TopLeftCorner || neighbor == RoomCell::TopRightCorner || neighbor == RoomCell::BottomLeftCorner || neighbor == RoomCell::BottomRightCorner || neighbor == RoomCell::DoorUpLeftCorner || neighbor == RoomCell::DoorUpRightCorner || neighbor == RoomCell::DoorDownLeftCorner || neighbor == RoomCell::DoorDownRightCorner || neighbor == RoomCell::DoorLeftCorner || neighbor == RoomCell::DoorRightCorner; };
            ...
            if (topWall && leftWall)
                return tileSet.backTopLeftCornerFrameName;
            if (topWall && rightWall)
                return tileSet.backTopRightCornerFrameName;
            if (bottomWall && leftWall)
                return tileSet.backBottomLeftCornerFrameName;
            if (bottomWall && rightWall)
                return tileSet.backBottomRightCornerFrameName;
            if (topWall)
                return tileSet.backTopFrameName;
            if (bottomWall)
                return tileSet.backGroundFrameName;
            if (leftWall)
                return tileSet.backLeftFrameName;
            if (rightWall)
                return tileSet.backRightFrameName;
        ...
        if (hasDoor(info.doorPositions, DoorPosition::Up) && y == topWallY + 1) {
            ...
            if (x == centerStart)
                return tileSet.backDoorTopLeftFrameName;
            if (x == centerEnd)
                return tileSet.backDoorTopRightFrameName;
        ...
        if (hasDoor(info.doorPositions, DoorPosition::Down) && y == groundY - 1) {
            ...
            if (x == centerStart)
                return tileSet.backDoorBottomLeftFrameName;
            if (x == centerEnd)
                return tileSet.backDoorBottomRightFrameName;
            if (x > centerStart && x < centerEnd)
                return tileSet.backInnerFrameName;
        }
        ...
        const bool topWall = top == RoomCell::Ceiling || top == RoomCell::TopLeftCorner || top == RoomCell::TopRightCorner;
        const bool bottomWall = bottom == RoomCell::Ground || bottom == RoomCell::BottomLeftCorner || bottom == RoomCell::BottomRightCorner;
    ...
    if (hasDoor(info.doorPositions, DoorPosition::Up)) {
        doorPlacements.push_back({{(centerStart + topBottomPassageWidth * 0.5f) * tileSize.x, topPassageInnerY}, 180.f});
    }
    if (hasDoor(info.doorPositions, DoorPosition::Down)) {
        doorPlacements.push_back({{(centerStart + topBottomPassageWidth * 0.5f) * tileSize.x, bottomPassageInnerY}, 0.f});
    }
    if (hasDoor(info.doorPositions, DoorPosition::Left)) {
        doorPlacements.push_back({{leftPassageInnerX, (sideDoorTop + sidePassageHeight * 0.5f) * tileSize.y}, 90.f});
    }
    if (hasDoor(info.doorPositions, DoorPosition::Right)) {
        doorPlacements.push_back({{rightPassageInnerX, (sideDoorTop + sidePassageHeight * 0.5f) * tileSize.y}, -90.f});
    }
...
std::optional<sf::Vector2f> Room::getPlayerSpawnPosition(const TileMap &tileMap) const {
    ...
    const sf::Vector2f tileSize = tileMap.getTileSize();
    return sf::Vector2f{(static_cast<float>(info.playerSpawnCell->x) + 0.5f) * tileSize.x, (static_cast<float>(info.playerSpawnCell->y) + 1.f) * tileSize.y};
}
...
std::vector<sf::Vector2f> Room::getMonsterSpawnPositions(const TileMap &tileMap) const {
    ...
    for (unsigned int y = 1; y < info.layout.height - 1; ++y) {
        for (unsigned int x = 1; x < info.layout.width - 1; ++x) {
            ...
            if ((cell == RoomCell::BackTile || cell == RoomCell::SpawnPoint) && (below == RoomCell::Ground || below == RoomCell::Platform)) {
                positions.push_back({(static_cast<float>(x) + 0.5f) * tileSize.x, (static_cast<float>(y) + 1.f) * tileSize.y});
            }
        }
    }
    return positions;
...
RoomLayout Room::getReferenceLayout(RoomType type) {
    ...
    switch (type) {
    case RoomType::Start:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{11, 6, 3}, {6, 9, 3}, {17, 9, 3}});
    case RoomType::Monster:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{10, 5, 5}, {6, 8, 3}, {18, 8, 3}, {11, 10, 4}});
    case RoomType::Trial:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{6, 4, 3}, {18, 6, 3}, {7, 8, 3}, {18, 10, 3}});
    case RoomType::Monster2:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{11, 4, 3}, {6, 7, 3}, {17, 9, 3}});
    case RoomType::Monster3:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{6, 4, 3}, {18, 6, 3}, {10, 8, 5}, {6, 10, 3}, {18, 10, 3}});
    case RoomType::Monster4:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{11, 5, 3}, {6, 7, 3}, {19, 7, 3}, {11, 9, 3}, {6, 11, 3}, {19, 11, 3}});
    case RoomType::Monster5:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{7, 5, 3}, {18, 5, 3}, {10, 7, 5}, {7, 9, 3}, {18, 9, 3}});
    case RoomType::Hut:
        return makeStyledRoom(kRegularRoomWidth, kRoomHeight, {{10, 5, 5}, {6, 7, 5}, {13, 7, 5}, {12, 10, 3}});
    case RoomType::Boss:
