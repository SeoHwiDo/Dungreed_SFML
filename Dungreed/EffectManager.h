...
class EffectManager {
  public:
    ...
    void spawnPlayerSwing(ObjectPoolingManager &objectPool, const sf::Vector2f &playerPosition, float aimRadian, float damage);
    /// 몬스터 피격 지점에 짧게 재생되는 SlashFX를 대여합니다.
    void spawnHitSlash(ObjectPoolingManager &objectPool, const sf::Vector2f &hitPosition, float rotationRadian);
    /// 몬스터가 등장하는 위치에 반투명 MagicCircleFx를 겹쳐 재생합니다.
    /// 매직서클 중간 프레임이 도달하는 시간을 반환합니다.
    float spawnMonsterMagicCircle(ObjectPoolingManager &objectPool, const sf::Vector2f &monsterPosition);
    /// 전투방 클리어 보물상자가 나타나는 위치에 불투명한 마법진을 재생합니다.
    void spawnRewardChestMagicCircle(ObjectPoolingManager &objectPool, const sf::Vector2f &chestPosition);
